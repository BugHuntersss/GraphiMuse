...
public class SingleAPIMiner implements AUGMiner {
    ...
    public SingleAPIMiner(Configuration config, String API) {
    ...
    public Model mine(Collection<APIUsageExample> examples) {
        return mine(new ArrayList<>(examples), api);
    }

    private Model mine(ArrayList<APIUsageExample> examples, String api) {
        ...
        try {
            ...
            return toModel(miner.mine(examples, api));
        } finally {
    ...
    private Model toModel(Set<Pattern> patterns) {
        return () -> patterns.stream().map(SingleAPIMiner::toAUGPattern).collect(Collectors.toSet());
    }
    ...
    private static APIUsagePattern toAUGPattern(Pattern pattern) {
        ...
        APIUsagePattern augPattern = new APIUsagePattern(pattern.getFreq(), exampleLocations, pattern.abstractNodeListForThis
                , pattern.APIFrequency);
