...
public class Overlap {
    ...
    public Set<String> violationFromDocumentCheck = new HashSet<>();
    ...
    public void addViolationFromDocument(String violation){
        this.violationFromDocumentCheck.add(violation);
    }
    ...
    public Set<String> getViolationFromDocumentCheck() {
        return violationFromDocumentCheck;
    }

    public void setViolationFromDocumentCheck(Set<String> violationFromDocumentCheck) {
        this.violationFromDocumentCheck = violationFromDocumentCheck;
    }

    public void addViolationFromDoc(String violation){
        if(violation != null) this.violationFromDocumentCheck.add(violation);
    }

    public double getProbability(){
        double pro = 1.0;
        Set<Node> missingNodes = getMissingNodes();
        for (Node missingNode : missingNodes) {
            if(missingNode.getConfidence() > 0) return 0.9999;
        }
        for (Node node : pattern.bayesMatrix.keySet()) {
            Map<UnionCondition, Double> map = pattern.bayesMatrix.get(node);
            List<Node> inN = pattern.inN.get(node);
            List<Boolean> practice = new ArrayList<>();
            for (int i = 0; i < inN.size(); i++) {
                practice.add(!missingNodes.contains(inN.get(i)));
            }
            UnionCondition unionCondition = new UnionCondition(practice);
            if(unionCondition.condition != 0){
                if(!missingNodes.contains(node)) pro = pro * map.get(unionCondition);
                else pro = pro * (1 - map.get(unionCondition));
            }
