...
public class AlternativeMappingsOverlapsFinder implements OverlapsFinder {
    ...
    private static class ExtensionStrategy {
        ...
        private Set<Alternative> getAlternatives(Node targetNode, APIUsagePattern pattern) {
            return pattern.vertexSet().stream()
                    .filter(patternNode -> abstractNodeMatcher(targetNode, patternNode, pattern))
                    .map(Alternative::new).collect(Collectors.toSet());
        }

        private boolean abstractNodeMatcher(Node targetNode, Node patternNode, APIUsagePattern pattern){
            if(patternNode instanceof MethodCallNode && targetNode instanceof MethodCallNode){
                //get pattern node label
                String label_node = ((MethodCallNode)patternNode).getDeclaringTypeName() + "." + ((MethodCallNode)patternNode).getMethodSignature();
                //get abstract node list
                ArrayList<String> nodeList = pattern.getAbstractNodeListForMethodCall().get(label_node);
                //get target node label
                String targetNodeLabel = ((MethodCallNode)targetNode).getDeclaringTypeName() + "." + ((MethodCallNode)targetNode).getMethodSignature();
                if(nodeList == null){
                    return config.nodeMatcher.test(targetNode, patternNode);
                }
                for(String label : nodeList){
                    if(targetNodeLabel.equals(label)) return true;
                }
                return false;
            }
            else return config.nodeMatcher.test(targetNode, patternNode);
            }
    ...
    public List<Overlap> findOverlaps(APIUsageExample target, APIUsagePattern pattern) {
        ...
        //Startnodes are nodes which is methodcall node and is not constructor node in the meantime
