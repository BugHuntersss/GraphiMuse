...
public class PAMDetectTest {
    ...
    @Test
    public void findsViolations() throws IOException, InterruptedException {
        ...
        PAMDetect PAMDetect = new PAMDetect(model, overlapsFinder, violationPredicate, rankingStrategy);
        List<Violation> violations = PAMDetect.findViolations(targets);
    ...
    @Test
    public void ignoresNonViolations() throws IOException, InterruptedException {
        final APIUsagePattern pattern = somePattern();
        final APIUsageExample target = someAUG();
        final Collection<APIUsageExample> targets = singletonList(target);
        final Overlap overlap = someOverlap(pattern, target);
