...
public class APIUsageGraph extends DirectedMultigraph<Node, Edge> {
    ...
    public Set<MethodCallNode> getMethodCallNode(String API){
        return vertexSet().stream()
                .filter((Node node) -> node.getAPI().isPresent() && node.getAPI().get().equals(API))
                .filter((Node node) -> node instanceof MethodCallNode)
                .map((Node node) -> (MethodCallNode)node)
                .collect(Collectors.toSet());
    }
    ...
    public Set<String> getMethodCalls() {
        return vertexSet().stream()
                ...
                .map((Node node) -> (MethodCallNode) node )
                .map(MethodCallNode::getMethodSignature)
                .collect(Collectors.toSet());
    }

    public Set<String> vertexLabelSet(){
