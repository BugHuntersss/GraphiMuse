...
public class TestAUGBuilder {
    ...
    public TestAUGBuilder withActionNode(String id, final String nodeName) {
        if (nodeName.equals("=")) {
            ...
            if (nodeName.contains(".")) {
                ...
                return withNode(id, new MethodCallNode(nameParts[0], nameParts[1],new ArrayList<>()));
            } else {
                return withNode(id, new MethodCallNode("", nodeName, new ArrayList<>()) {
                    @Override
    ...
    public <T extends APIUsageGraph> T build(Class<T> clazz) {
        ...
        if (clazz == APIUsageExample.class) {
            ...
            aug = new APIUsagePattern(42, new HashSet<>(),new HashMap<>(),new HashMap<>());
        } else {
