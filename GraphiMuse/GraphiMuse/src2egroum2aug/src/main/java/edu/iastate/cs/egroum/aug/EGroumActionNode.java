...
public class EGroumActionNode extends EGroumNode {
	...
	public EGroumActionNode(EGroumNode control, String branch, ASTNode astNode, int nodeType, String key, String type, String name) {
		...
		if(name.equals("iterator"))
			this.setReturnTypes("public interface Iterator<java.lang.String>");
		else if(name.equals("hasNext"))
			this.setReturnTypes("Boolean");
		...
		if(astNode instanceof MethodInvocation){
			...
			if (node.resolveMethodBinding() != null) {
				...
				for (ITypeBinding parameterType : mb.getParameterTypes()) {
					this.parameters.add(parameterType.getName());
				}

				this.setReturnTypes(mb.getReturnType().toString().split("\n")[0]);
			}
		}
	}
	...
	public EGroumActionNode(EGroumActionNode node) {
		...
		this.returnTypes = node.returnTypes;
	}
