...
public class EGroumBuilder {
	...
	private final HashSet<String> APISet;
	private File commentOutput = new File(System.getProperty("user.dir") + "/comment.txt");
    
    public EGroumBuilder(AUGConfiguration configuration, HashSet<String> APISet) {
		this.APISet = APISet;
	 ...
	 */
	public ArrayList<EGroumGraph> buildBatch(String path, String[] classpaths) throws IOException {
		buildStandardJars();
	 ...
	 *            i.e., hundreds of times for large project such as JDT core.
	 */
	public ArrayList<EGroumGraph> build(String path, String[] classpaths) throws IOException {
		buildStandardJars();
		buildHierarchy(new File(path));
	...
	private ArrayList<EGroumGraph> buildBatchGroums(File dir, String[] classpaths) throws IOException {
		...
		FileASTRequestor r = new FileASTRequestor() {
			...
			public void acceptAST(String sourceFilePath, CompilationUnit cu) {
				if (configuration.usageExamplePredicate.matches(sourceFilePath, cu) && sourceFilePath!=null)
		...
		String[] newPaths = Arrays.stream(paths).filter(Objects::nonNull).toArray(String[]::new);
		parser.createASTs(newPaths, null, new String[0], r, null);
		ArrayList<EGroumGraph> groums = new ArrayList<>();
		FileWriter out = new FileWriter(commentOutput, false);
		out.write(" ");
		out.close();
			...
			for (int i = 0 ; i < cu.types().size(); i++){
				if (cu.types().get(i) instanceof TypeDeclaration){
					if(cu.getPackage() != null){
						groums.addAll(buildGroums((TypeDeclaration) cu.types().get(i), path, "", cu.getPackage().toString()));
					}
					else{
						groums.addAll(buildGroums((TypeDeclaration) cu.types().get(i), path, "", null));
					}
					if(groums.size()==761)
						System.out.println();
				}
	...
	private ArrayList<EGroumGraph> buildGroums(File file, String[] classpaths) throws IOException {
		ArrayList<EGroumGraph> groums = new ArrayList<>();
		if (file.isDirectory()) {
			for (File sub : file.listFiles())
	...
	private ArrayList<EGroumGraph> buildGroums(TypeDeclaration type, String path, String prefix, String packageName) throws IOException {
		ArrayList<EGroumGraph> groums = new ArrayList<>();
		for (MethodDeclaration method : type.getMethods())
			if (configuration.usageExamplePredicate.matches(method)) {
				ASTNode parent = method.getParent();
				FileWriter out = new FileWriter(commentOutput, true);
				String line = System.getProperty("line.separator");
				out.write(line);
				out.write("PACKAGE : " + packageName);
				out.write(line);
				out.write("NAME : " + method.getName());
				out.write(line);
				out.write("COMMENT : " + method.getJavadoc());
				out.write(line);
				out.write(line);
				out.close();
				EGroumGraph g = buildGroum(method, path, prefix + type.getName().getIdentifier() + ".");
				if (configuration.usageExamplePredicate.matches(g) /* && check(g)*/)
					groums.add(g);
				//else g = null;
			}
		for (TypeDeclaration inner : type.getTypes())
			groums.addAll(buildGroums(inner, path, prefix + type.getName().getIdentifier() + ".", packageName));
		return groums;
	}
	...
	Boolean check(EGroumGraph g){
		for(EGroumNode node: g.getNodes()){
			for(String API : this.APISet) {
				if (node.dataType != null && node.dataType.contains(API)) return true;
			}
		}
		return false;
	}
